function compare_mod_sam_for_draft_20201209(mod1,mod2,varargin)
...
% compare_mod_sam_for_draft_20201209(81,88);
...
%tex_variables; % add lookup table for tex variable names for vars
tex_variables_L_and_H; % use L and H labels instead
...
if length(varargin)==3
    ...
    % match what it is BCLR draft from summer 2020
...
%       irf_vars={{'dya';'dgdp';'dyp';'dpgdp';'dyc'},{'dip';'ds';'igitot';'ntotal';'nc';'np';'ncnt'}};
%       irf_vars={{'Er_H_patent';'Erc';'titot'}};

%     irf_vars={{'dgdp';'da';'dyp';'dc';'dip';'ds';'igitot';'qp';'VexpK';'p'}};
...
for j=1:length(irf_vars) %% Loop over lists within irf_vars
    ...
    set(gcf, 'PaperPosition', [0 0 8.00 11.00]);
...
%                 end
            ...
            if k==1, title(MyTitles{1},'interpreter','latex','FontSize', 14); end            
            %if k==length(temp_irf), xlabel('Quarters'); end
            if k==length(temp_irf), xlabel('Quarters','FontSize', 14); end
...
%                 end   
            ...
            if k==1, title(MyTitles{2},'interpreter','latex','FontSize', 14); end            
            %if k==length(temp_irf), xlabel('Quarters'); end
            if k==length(temp_irf), xlabel('Quarters','FontSize', 14); end
...
%             end;
    ...
    fname=strcat([num2str(mod1),'_vs_',num2str(mod2),'_',shocks{1},'_',shocks{2},'_irf', num2str(j)]);    
