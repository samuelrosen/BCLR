...
% Pi = 1;
% alphac = 0.999;%0.99
global arc alphap alphac phi_growth rhoar deltak deltac mar Lss gamma xi cxi sigma sigmax sigmas rhovol ar_1_switch b_ma beta croce_switch delta eps f gam labor_switch mu Nbar phi psi rho rho_sigma stoch_vol_switch tau tca_productivity_switch v varphi vcov ORDER
global gold_switch 
%% ===========================================================
...
qss = 1;                                        % marginal q
Kss = ((Rss+deltak-1)/(w*((1-1/tau)*alphap+(1-alphap)*phi_growth)*(1-Lss)^(1-alphap)*exp(mu)^(1-alphap)))^(1/(alphap-1));    % capital
Kcss = ((Rcss+deltac-1)/((1-w)*(alphac-(1-alphac)*rhoar*phi_growth)*exp(mu+arc)^(1-alphac)))^(1/(alphac-1));
...
%% The Function
[x,FVAL,S,N] = fsolve(@(x) myfun2(x, mu, rho, beta, psi, gamma, Lss, alphac, alphap, deltak, deltac, xi, tau, arc, w, Gprimss, Gss, philev, rhoar, phi_growth),X, optimset('MaxFunEvals', 3000000,'MaxIter',100000,'Display','off'));
