function compare_mod(mod1,mod2,varargin)

close all

addpath(genpath('Safe Capital Model 3'))
clearvars -global -except mod1 varargin; clc;

COLUMNS =1; % By default, only 2 shocks and hence 2 cols of IRFs
...
if length(varargin)>=2
  shocks = {varargin{1}; varargin{2}; };
    ...
    disp('Using default shocks ea and ex');
    shocks = {'ea'; 'ex'; 'es'};
end
...
for z=1:COLUMNS
    if strcmp(shocks(z),'ea')
        MyTitles{z}='Short-Run Shock, Long-Run Shock and Uncertainty Shock';
...
if length(varargin)==1
    ...
    irf_vars={{'dytotalg';'dc';'di';'tig';'dic';'Ic';'kratio'}};
...
for j=1:length(irf_vars) %% Loop over lists within irf_vars
    ...
    for i=1:3 %% Loop over models
        ...
        if i==1
            ...
            load(strcat('Results/SafeCapital_Model_3_Approx_',num2str(mod1),'/SafeCapital_Model_3_Approx_',num2str(mod1),'.mat'));
            style='-k';
        elseif i==2    
            load(strcat('Results/SafeCapital_Model_3_Approx_',num2str(mod1),'/SafeCapital_Model_3_Approx_',num2str(mod1),'.mat'));
            style='-b';
        ...
        for k=1:length(temp_irf)
            ...
            box on; hold on;
            if i~=0 && exist(strcat(['dyn_irfp_',shocks{i},'_mean'])) % also checks if shock is in calibration
%                 if strcmp(var_first_5,'condE') % if conditional expected value
%                     h(i)=plot(1:irf_sim,[eval(strcat(['irf_condE_',shocks{i},'(dyn_i_',temp_irf{k}(7:end),',1:irf_sim)']))],style,'LineWidth',2);
%                 else
%                     h(i)=plot(1:irf_sim,[eval(strcat(['dyn_irfp_',shocks{i},'_mean(dyn_i_',temp_irf{k},',1:irf_sim)']))],style,'LineWidth',2);
%                 end
                if strcmp(var_first_5,'condE') % if conditional expected value
                    h(i)=plot(1:irf_sim,[eval(strcat(['irf_condE_',shocks{i},'(dyn_i_',temp_irf{k}(7:end),',1:irf_sim)']))],style,'LineWidth',2);
                elseif strcmp(var_first_5,'condS') % if conditional standard deviation
                    h(i)=plot(1:irf_sim,[eval(strcat(['irf_condStd_',shocks{i},'(dyn_i_',temp_irf{k}(9:end),',1:irf_sim)']))],style,'LineWidth',2);
                elseif strcmp(var_first_5,'corr_') || strcmp(var_first_5,'covar') % if conditional correlation or covariance
                    for w=1:irf_sim
                        corr(w) = eval(strcat(['irf_condCorr_',shocks{i},'(dyn_i_',corr_var1,',dyn_i_',corr_var2,',w)']));
                        corr_check(w) = eval(strcat(['irf_condCorr_',shocks{i},'(dyn_i_',corr_var2,',dyn_i_',corr_var1,',w)']));
                        std_var1(w)= eval(strcat(['irf_condStd_',shocks{i},'(dyn_i_',corr_var1,',w)'])) + std_var1_start;
                        std_var2(w)= eval(strcat(['irf_condStd_',shocks{i},'(dyn_i_',corr_var2,',w)'])) + std_var2_start;
                    end
...
%             counter=counter+1;
            ...
            if k==length(temp_irf) && i==3 
                legend(h(1:3),'Short-Run Shock', 'Long-Run Shock', 'Uncertainty Shock');
    ...
    fname=strcat([num2str(mod1),'_vs_',num2str(mod2),'_',shocks{i},'_',shocks{2},'_irf', num2str(j)]);    
