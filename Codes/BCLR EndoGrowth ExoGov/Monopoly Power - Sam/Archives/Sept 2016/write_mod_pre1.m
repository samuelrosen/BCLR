...
fprintf(fid, '\n');
fprintf(fid, 'parameters deltac mu rho rhovol rhoar psi gamma alphap deltak xi cxi alpha1 alpha0 calpha1 calpha0 philev sigma sigmax alphac beta app tau w Np Nc arc con_shocka con_shockx phi_growth pss ptilss ypss yss ipss kpss rhoinv phi_growth_f;\n');
fprintf(fid, '\n');
...
fprintf(fid, 'qp = (ptil-((1-alphap)*(1/tau)*p*(1-rhoar)*phi_growth*exp(ypa-ipa))-(p*(1-alphap)*exp(ypa)*phi_growth*exp(-ipa)))/(Gampprim);\n');
...
if stoch_vol_switch==1;
fprintf(fid, 'da = mu + x(-1) + exp(vol(-1))*ea+ phi_growth*((log(exp(ipa))-log(exp(ypa)))-(log(exp(ipss))-log(exp(ypss)))) ;\n');
if volx ==1;
