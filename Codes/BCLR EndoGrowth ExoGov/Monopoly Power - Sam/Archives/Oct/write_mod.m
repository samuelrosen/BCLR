...
global gold_switch gov_adjustment_switch phi_growth
calibrations;
%% ===========================================================
% Preliminaries 
%=============================================================
...
fprintf(fid, '\n');
fprintf(fid, 'parameters deltac mu rho rhovol rhoar psi gamma alphap deltak xi cxi alpha1 alpha0 calpha1 calpha0 philev sigma sigmax alphac beta app tau w Np Nc arc con_shocka con_shockx phi_growth pss ptilss ypss yss ipss;\n');
fprintf(fid, '\n');
...
fprintf(fid, '//Returns;\n');%7
fprintf(fid, 'exp(r) = ((1-1/tau)*p*alphap*exp(ypa-kpa(-1)+da)+qp*(1-deltak)+qp*(-Gampprim*exp(ipa-kpa(-1)+da)+Gamp))/(qp(-1));\n');
...
if stoch_vol_switch==1;
fprintf(fid, 'da = mu + x(-1) + exp(vol(-1))*ea + phi_growth*((ipa-ya)-(ipss-yss));\n');
if volx ==1;
...
if stoch_vol_switch==0;
    fprintf(fid, 'da = mu + x(-1) + ea + phi_growth*((log(exp(ipa))-log(exp(ypa)))-(log(exp(ipss))-log(exp(ypss))));\n');
    fprintf(fid, 'x = rho*x(-1) + ex;\n');
end
% if rhotest == 0;
