...
fprintf(fid, 'var EV ut da argp ev ca dgdpa dgdppa dfyp dfyc dgdpp dycp ypa yca ya kpa kca ipa Ic Vexp dVexp D Tax G Gprim T Tprim Gamp Gampprim Gamc Gamcprim m mc dc qp qc p ptil r rc re x dyp dyc dyvp dip igitot exr exr_G rf vol eng Eng Engma kratio gratio wratio yratio tyg tig gammav dy pqr wagep wagec Vr EMVr qpt VexpK ikp iyp ar_inv dkp ia x_f kpkg tca dtc np nc ntotal npnt ncnt sla LC dLCdHP dLCdHG wagepc w_var;\n');%Vr EMVr 
...
fprintf(fid, 'ypa = alphap*(kpa(-1)-da)+(1-alphap)*log((exp(np)));\n');
fprintf(fid, 'yca = alphac*(kca(-1)-da)+(1-alphac)*(log((exp(nc)))+argp);\n');
...
fprintf(fid, 'exp(ya)= exp(ca)+exp(ipa)+Ic+LC;\n');
...
fprintf(fid, 'wagep = p*(1-1/tau)*(1-alphap)*exp(ypa)/(exp(np));\n');
...
fprintf(fid, 'wagepc=%3.6f;\n', (pss*(1-1/tau)*(1-alphap)*exp(ypss)/Np_ss)/((1-alphac)*exp(ycss)/Nc_ss));
fprintf(fid, 'D=%3.6f;\n', (pss*exp(ypss)-(pss*(1-1/tau)*(1-alphap)*exp(ypss)/Np_ss)*Np_ss-ptilss*exp(ipss))/ptilss);
fprintf(fid, 'Tax=%3.6f;\n', -(exp(ycss)-((1-alphac)*exp(ycss)/Nc_ss)*Nc_ss-ptilss*exp(icss))/ptilss);
