// this program creates some of the figures and results that we use in our
// response to the JPE referees
...
/*********************************************************************************************/			
// relative size of federal govt defense investment 

// quarterly

	use "${dtapath}\bea_quarterly", replace		
	tsset dateqtr

	gen fed_defense_as_pct_govt_inv = 100*gross_govt_inv_fed_def_nom / gross_govt_inv_nom
	tsline fed_defense_as_pct_govt_inv, graphregion(color(white)) ///
		lpattern(solid dash solid) lcolor(black blue red) ///
		lwidth(0.7 0.7 0.7) ///
		///xline(1972, lpattern(dash) lcolor(black)) ///
		title("Federal National Defense Investment as Percent of Total Govt. Inv.") ///
		ytitle("Percent")
	graph export "${outFigure}\tsline_fed_defense_as_pct_govt_inv.png", replace	
	
	gen ig_to_gdp = 100*gross_govt_inv_nom / gdp_nom
	gen ig_less_def_to_gdp = 100*(gross_govt_inv_nom-gross_govt_inv_fed_def_nom) / gdp_nom
	label var ig_to_gdp "IgTot / GDP"
	label var ig_less_def_to_gdp "(IgTot - IgDef) / GDP"
	tsline ig_to_gdp ig_less_def_to_gdp, graphregion(color(white)) ///
		lpattern(solid dash solid) lcolor(black blue red) ///
		lwidth(0.7 0.7 0.7) ///
		///xline(1972, lpattern(dash) lcolor(black)) ///
		title("Federal National Defense Investment as Percent of Total Govt. Inv.") ///
		ytitle("Percent")
	graph export "${outFigure}\tsline_ig_to_gdp_vs_ig_less_def_to_gdp.png", replace		
	
	keep if dateqtr<=yq(2016,4)
	collapse (mean) ig_to_gdp ig_less_def_to_gdp
	list	
	
// annual

	use "${dtapath}\bea_annual", replace		
	tsset year

	gen fed_defense_as_pct_govt_inv = 100*gross_fed_govt_inv_nom_def / gross_govt_inv_nom
	tsline fed_defense_as_pct_govt_inv, graphregion(color(white)) ///
		lpattern(solid dash solid) lcolor(black blue red) ///
		lwidth(0.7 0.7 0.7) ///
		///xline(1972, lpattern(dash) lcolor(black)) ///
		title("Federal National Defense Investment as Percent of Total Govt. Inv.") ///
		ytitle("Percent")
	graph export "${outFigure}\tsline_annual_fed_defense_as_pct_govt_inv.png", replace	
	
	gen ig_to_gdp = 100*gross_govt_inv_nom / gdp_nom
	gen ig_less_def_to_gdp = 100*(gross_govt_inv_nom-gross_fed_govt_inv_nom_def) / gdp_nom
	label var ig_to_gdp "IgTot / GDP"
	label var ig_less_def_to_gdp "(IgTot - IgDef) / GDP"
	tsline ig_to_gdp ig_less_def_to_gdp, graphregion(color(white)) ///
		lpattern(solid dash solid) lcolor(black blue red) ///
		...
		title("Federal National Defense Investment as Percent of Total Govt. Inv.") ///
		ytitle("Percent")
	graph export "${outFigure}\tsline_annual_ig_to_gdp_vs_ig_less_def_to_gdp.png", replace	
	
	keep if year>=1929 & year<=2016
	collapse (mean) ig_to_gdp ig_less_def_to_gdp
	list
 ...
 // choices for potential profitability controls: markup_ebitda_F0, cfprof_ratio 
  foreach new_export_sheet in 1 { 
   foreach startyr in 1972 1975 1982 { //    
    foreach vol_var in "ivol" "epu" { // "ivol" "epu" run for both 
